module.exports = ({ time, response, responseDM, run }) => {
  const requirement = {
    validate: (msg, args, client, command, req) => {
      ...
      const cooldown =
        (req.cooldowns[msg.channel.id] || 0) -
        Math.round(new Date().getTime() / 1000);
      args.reqChannelCooldown = {
        cooldown,
  ...
  if (response) {
    if (typeof response === 'string') {
      requirement.response = (msg, args, client, command, req) =>
        replacement(response, args.reqChannelCooldown);
  ...
  if (responseDM) {
    if (typeof responseDM === 'string') {
      requirement.responseDM = (msg, args, client, command, req) =>
        replacement(responseDM, args.reqChannelCooldown);
    } else if (typeof responseDM === 'function') {
      requirement.responseDM = (msg, args, client, command, req) =>
        replacement(
          responseDM(msg, args, client, command, req),
          args.reqChannelCooldown
        );
    }
  }
